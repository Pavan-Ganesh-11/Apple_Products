...
const AllRecipeItems = () => {
  ...
  const recipeData = async () => {
    ...
    const recipeUrl = `http://localhost:3000/VEGRECIPES/${id}`;
    const options = {
      method: "GET",
    };
    ...
    return (
      <>
        <img
          src={recipeImageUrl}
          alt={recipeName}
          className="all-veg-recipe-item-img"
        />
        <div className="all-veg-recipe-details-container">
          <h1 className="all-veg-recipe-name">{recipeName}</h1>
          <p className="all-veg-recipe-desc">{recipeDesc}</p>
          <hr className="all-veg-recipe-hr" />
          <div className="all-veg-recipe-ingredients">
            <h1 className="all-veg-recipe-ingredients-heading">Ingredients</h1>
            <ul className="all-veg-recipe-ingredients-list-wise">
              <>
                {ingredients.map((eachItem) => (
                  <li className="all-veg-recipe-item">{eachItem.ingredient}</li>
                ))}
              </>
            </ul>
          </div>
          <hr className="all-veg-recipe-hr" />
          <p className="all-veg-recipe-price">
            <span className="veg-price">Price: </span>
            <FaRupeeSign size={18} color="#555" />
            {recipePrice}
          </p>
          <p className="veg-stock">
  ...
  const recipeItemFailure = () => {
    return (
      <div className="all-veg-recipe-failure-container">
        <img
          src="https://assets.ccbp.in/frontend/react-js/nxt-watch-failure-view-light-theme-img.png"
          alt="Recipe Failure"
          className="all-veg-recipe-falilure-img"
        />
        <h1 className="all-veg-recipe-failure-heading">
          Oops! Something Went Wrong
        </h1>
        <p className="all-veg-recipe-failure-desc">
        ...
        <button
          type="button"
          className="all-veg-recipe-failure-button btn btn-primary"
          onClick={() => onRetry()}
        >
          Retry
        </button>
  ...
  return (
    <>
      <Header />
      <div className="all-veg-recipes-container">{switchItem()}</div>
