...
import "./AllSnacksRecipeItems.css";
...
const AllRecipeItems = () => {
  ...
  const recipeData = async () => {
    ...
    const recipeUrl = `http://localhost:3000/SNACKS/${id}`;
    const options = {
      method: "GET",
    };
    ...
    return (
      <>
        <img
          src={recipeImageUrl}
          alt={recipeName}
          className="all-snacks-recipe-item-img"
        />
        <div className="all-snacks-recipe-details-container">
          <h1 className="all-snacks-recipe-name">{recipeName}</h1>
          <p className="all-snacks-recipe-desc">{recipeDesc}</p>
          <hr className="all-snacks-recipe-hr" />
          <div className="all-snacks-recipe-ingredients">
            <h1 className="all-snacks-recipe-ingredients-heading">
              Ingredients
            </h1>
            <ul className="all-snacks-recipe-ingredients-list-wise">
              <>
                {ingredients.map((eachItem) => (
                  <li className="all-snacks-recipe-item">
                    {eachItem.ingredient}
                  </li>
                ))}
              </>
          ...
          <hr className="all-snacks-recipe-hr" />
          <p className="all-snacks-recipe-price">
            <span className="snacks-price">Price: </span>
            <FaRupeeSign size={18} color="#555" />
            {recipePrice}
          </p>
          <p className="snacks-stock">
  ...
  const recipeItemFailure = () => {
    return (
      <div className="all-snacks-recipe-failure-container">
        <img
          src="https://assets.ccbp.in/frontend/react-js/nxt-watch-failure-view-light-theme-img.png"
          alt="Recipe Failure"
          className="all-snacks-recipe-falilure-img"
        />
        <h1 className="all-snacks-recipe-failure-heading">
          Oops! Something Went Wrong
        </h1>
        <p className="all-snacks-recipe-failure-desc">
        ...
        <button
          type="button"
          className="all-snacks-recipe-failure-button btn btn-primary"
          onClick={() => onRetry()}
        >
          Retry
        </button>
  ...
  return (
    <>
      <Header />
      <div className="all-snacks-recipes-container">{switchItem()}</div>
