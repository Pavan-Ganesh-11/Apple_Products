...
import "./AllStaterRecipeItems.css";
...
const AllRecipeItems = () => {
  ...
  const recipeData = async () => {
    ...
    const recipeUrl = `http://localhost:3000/STATERS/${id}`;
    const options = {
      method: "GET",
    };
    ...
    return (
      <>
        <img
          src={recipeImageUrl}
          alt={recipeName}
          className="all-stater-recipe-item-img"
        />
        <div className="all-stater-recipe-details-container">
          <h1 className="all-stater-recipe-name">{recipeName}</h1>
          <p className="all-stater-recipe-desc">{recipeDesc}</p>
          <hr className="all-stater-recipe-hr" />
          <div className="all-stater-recipe-ingredients">
            <h1 className="all-stater-recipe-ingredients-heading">
              Ingredients
            </h1>
            <ul className="all-stater-recipe-ingredients-list-wise">
              <>
                {ingredients.map((eachItem) => (
                  <li className="all-stater-recipe-item">
                    {eachItem.ingredient}
                  </li>
                ))}
              </>
          ...
          <hr className="all-stater-recipe-hr" />
          <p className="all-stater-recipe-price">
            <span className="stater-price">Price: </span>
            <FaRupeeSign size={18} color="#555" />
            {recipePrice}
          </p>
          <p className="stater-stock">
  ...
  const recipeItemFailure = () => {
    return (
      <div className="all-stater-recipe-failure-container">
        <img
          src="https://assets.ccbp.in/frontend/react-js/nxt-watch-failure-view-light-theme-img.png"
          alt="Recipe Failure"
          className="all-stater-recipe-falilure-img"
        />
        <h1 className="all-stater-recipe-failure-heading">
          Oops! Something Went Wrong
        </h1>
        <p className="all-stater-recipe-failure-desc">
        ...
        <button
          type="button"
          className="all-stater-recipe-failure-button btn btn-primary"
          onClick={() => onRetry()}
        >
          Retry
        </button>
  ...
  return (
    <>
      <Header />
      <div className="all-stater-recipes-container">{switchItem()}</div>
